...
<body>
  ...
  <script>
    ...
    // Device/browser as short ASCII slugs; country as the raw IANA timezone.
    // The BACKEND translates these to Arabic before writing the sheet, so NO
    // Arabic ever travels in the analytics URL (Arabic in the URL was breaking
    // the beacon while the short ASCII code-request kept working).
    ...
    // ---- Analytics: image beacon (most reliable fire-and-forget) -----------
    // Auto-fired cross-origin <script> beacons were being dropped on iOS Safari;
    // an <img> GET is the classic analytics pixel and reaches the backend
    // reliably. All params are ASCII; '_' is a cache-buster.
    function track(event) {
      try {
        var params = { api: 'log', event: event, d: META.d, b: META.b, c: META.c, s: META.s, _: Date.now() };
        var qs = Object.keys(params).map(function (k) {
          return encodeURIComponent(k) + '=' + encodeURIComponent(params[k]);
        }).join('&');
        (new Image()).src = API + '?' + qs;
      } catch (e) { /* ignore */ }
    }
